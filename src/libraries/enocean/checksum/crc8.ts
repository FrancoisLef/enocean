/**
 * Official CRC8 (Cyclic redundancy check) lookup table from EnOcean ESP3 specification
 *
 * CRC8 uses 9-bit polynomial length
 */
const CRC8_TABLE: readonly number[] = [
  ,
] as const;

/**
 * Calculates CRC8 for the given data buffer using lookup table
 * @param data - Data buffer to calculate CRC for
 * @returns CRC8 value
 */
export function calculateCrc8(data: Buffer): number {
  let crc = 0;

  for (const byte of data) {
    crc = CRC8_TABLE[crc ^ byte];
  }

  return crc;
}

/**
 * Verifies if the given data matches the expected checksum
 *
 * @param data - Data buffer to verify
 * @param expectedChecksum - Expected CRC8 checksum value
 * @returns true if checksum matches, false otherwise
 */
export function verifyCrc8(data: Buffer, expectedChecksum: number): boolean {
  const calculatedChecksum = calculateCrc8(data);
  return calculatedChecksum === expectedChecksum;
}
